...
namespace LinqExercise
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var orderNum = numbers.OrderBy(x => x).ToList();
            orderNum.ForEach(x => Console.WriteLine(x));
            Console.WriteLine("--------");
            orderNum.OrderByDescending(x => x).ToList().ForEach(x => Console.WriteLine(x));
            Console.WriteLine("--------");
            //Print to the console only the numbers greater than 6
            var aboveSix = orderNum.Where(x => x > 6).ToList();
            aboveSix.ForEach(x => Console.WriteLine(x));
            Console.WriteLine("--------");
            //Order numbers in any order (acsending or desc) but only print 4 of them **foreach loop only!**
            var onlyFour = numbers.OrderByDescending(x => x).Take(4).ToList();
            onlyFour.ForEach(x => Console.WriteLine(x));
            Console.WriteLine("--------");
            ...
            var ageList = employees.Where(x => x.Age > 26).OrderBy(x => x.Age).ThenBy(x => x.FirstName).ToList();
            ageList.ForEach(x => Console.WriteLine($"{x.FullName} {x.Age}"));
            Console.WriteLine("--------");
            ...
            var expList = employees.Where(x => x.YearsOfExperience <= 10 && x.Age > 35);
            ...
            employees = employees.Append(new Employee("Philip", "Goforth", 32, 5)).ToList();
